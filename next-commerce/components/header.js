...
function Header() {
    ...
    const {show,toggleShow,cartItems} = cartContext
    return (
        <>
        <header>
        <div className="container mx-auto px-6 py-3 ">
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center justify-end w-full">
                    {!show ? <button onClick={toggleShow} className="text-gray-600 fixed focus:outline-none mx-4 sm:mx-0">
                        <svg className="h-5 w-5" fill="none" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewBox="0 0 24 24" stroke="currentColor">
                            <path d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"></path>
                        </svg>
                        <span className='fixed -mt-2 ml-3  bg-blue-600 text-white rounded-full h-6 w-6'>{cartItems.length}</span>
                    </button>: <Checkout/>
                    }

                    <div className="flex sm:hidden">
                        <button onClick={toggleShow} type="button" className="text-gray-600 hover:text-gray-500 focus:outline-none focus:text-gray-500" aria-label="toggle menu">
            ...
            <nav className = {`${show} ? '' : sm:flex sm:justify-center sm:items-center mt-4`}>
                <div className="flex flex-col sm:flex-row">
                   <Link href='/'>
                       <a className="mt-3 text-gray-600 hover:underline sm:mx-3 sm:mt-0">Home</a>
                    </Link> 
              <Link href='/shop'><a className="mt-3 text-gray-600 hover:underline sm:mx-3 sm:mt-0">Shop</a></Link>  
